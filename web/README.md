...
**"Prêts à préparer"** : jusqu'ici, `availability === "ready"` (tous les ingrédients requis en stock "disponible", sans dégradation) n'existait qu'agrégé en un compteur dans l'anneau de compatibilité de Mon Bar — jamais sous forme de liste consultable. Mis en avant à deux endroits : sur l'accueil (`Home.tsx`), une section juste après la bannière "presque prêt", même style que les autres collections (carrousel horizontal, `CocktailCard`) ; dans Mon Bar (`MyBar.tsx`), une section juste au-dessus de "Presque prêt", même style de liste verticale (`MiniGlassBadge`). Masquées dans les deux cas quand Mon Bar est vide ou que rien n'est encore prêt.
