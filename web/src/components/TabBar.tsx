    ...
    >
      {TABS.map((tab) => (
        <NavLink
          ...
          className={({ isActive }) =>
            `flex-1 flex flex-col items-center gap-0.5 py-2.5 text-[11px] font-medium transition-opacity ${
              isActive ? "opacity-100" : "opacity-55"
            }`
          }
          style={({ isActive }) => ({ color: isActive ? "var(--color-accent-gold)" : "var(--color-text-secondary)" })}
        >
