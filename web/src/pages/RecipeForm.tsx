...
export default function RecipeFormPage() {
  ...
  function handleSave() {
    ...
    if (!trimmedName) return setError("Le nom du cocktail est obligatoire.");
    if (validIngredientRows.length === 0) return setError("Ajoute au moins un ingrédient.");
    if (validStepRows.length === 0) return setError("Ajoute au moins une étape de préparation.");
  ...
  return (
    <div className="pb-10">
      <ScreenHeader title={isEditing ? "Modifier la recette" : "Nouvelle recette"} />
      ...
      <div className="px-4 pt-3 flex flex-col gap-4">
        ...
        <Field label="Nom du cocktail">
          <TextInput value={name} onChange={setName} placeholder="Ex: Jardin Doré" />
        </Field>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Verre">
            <TextInput value={glassware} onChange={setGlassware} placeholder="Ex: Verre Highball" />
          </Field>
          <Field label="Glace">
            <TextInput value={iceType} onChange={setIceType} placeholder="Ex: Glaçons" />
          </Field>
        </div>

        <Field label="Garniture">
          <TextInput value={garnish} onChange={setGarnish} placeholder="Ex: Zeste de citron" />
        </Field>
            ...
            {ingredientRows.map((row) => (
              <div key={row.key} className="rounded-xl p-3 flex flex-col gap-2" style={{ background: "var(--color-surface)" }}>
                ...
                <div className="flex gap-2">
                  <TextInput value={row.quantity} onChange={(v) => updateIngredient(row.key, { quantity: v })} type="number" placeholder="Qté" />
                  <TextInput value={row.unit} onChange={(v) => updateIngredient(row.key, { unit: v })} placeholder="Unité" />
                </div>
          ...
          <div className="flex flex-col gap-2">
            {stepRows.map((row, idx) => (
              <div key={row.key} className="rounded-xl p-3 flex flex-col gap-2" style={{ background: "var(--color-surface)" }}>
                ...
                <TextInput value={row.instruction} onChange={(v) => updateStep(row.key, { instruction: v })} placeholder="Instruction" />
                <div className="flex items-center gap-2">
                  <TextInput
                    value={row.durationSeconds}
                    onChange={(v) => updateStep(row.key, { durationSeconds: v })}
                    type="number"
                    placeholder="Durée (secondes, optionnel)"
                  />
                  <button
                    type="button"
        ...
        <Field label="Histoire (optionnel)">
          <TextInput value={history} onChange={setHistory} placeholder="Un peu de contexte..." />
        </Field>
        <Field label="Origine (optionnel)">
          <TextInput value={origin} onChange={setOrigin} placeholder="Ex: France" />
        </Field>
        <Field label="Conseils (optionnel)">
          <TextInput value={tips} onChange={setTips} placeholder="Une astuce de préparation..." />
        </Field>
        ...
        >
          {isEditing ? "Enregistrer les modifications" : "Ajouter à ma bibliothèque"}
        </button>
      </div>
    </div>
  );
